<script lang="ts">
  ...
  import Content from '../../home-2/_components/Content.svelte'
  ...
  onMount(async () => {
    try {
      const { questions, error } = await getQuestionArrayByQuizId(
        quizId,
        supabase,
  ...
  async function uploadResponsesAndFetchCorrectAnswers() {
    try {
      const userId = data?.user?.id // Assuming you have user data in `data`
  ...
  async function getCorectAnswers() {
    try {
      quizId = Number($page.url.pathname.split('/').pop()) // Correctly access quizId from the URL
  ...
  function calculateScore() {
    ...
    correctAnswers.forEach((correctAnswer) => {
      const userAnswer = userResponses.find(
        //@ts-ignore
        (response) => response.question_id === correctAnswer.quiz_question_id,
      )
