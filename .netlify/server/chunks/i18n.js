...
const serializeRoute = (path, normalizedBase, dataSuffix) =>
...
function removeDataSuffix(absolutePath) {
  ...
  const dataSuffix = KNOWN_SUFFIXES.find((suffix) =>
    absolutePath.endsWith(suffix),
  )
  if (dataSuffix) {
...
function getHrefBetween(from, to) {
  ...
  if (to.password || to.username) {
    ...
    return (
      '//' + credentials + '@' + to.host + to.pathname + to.search + to.hash
    )
  }
...
const createHandle = (strategy, i18n2, options) => {
  ...
  function initializeLanguageContext(AsyncLocalStorage) {
    languageContext = AsyncLocalStorage
      ? new ALSContext(AsyncLocalStorage)
      : new GlobalContext()
    i18n2.runtime.setLanguageTag(() => {
      ...
      return i18n2.runtime.isAvailableLanguageTag(val)
        ? val
        : i18n2.defaultLanguageTag
    })
  }
  const langPlaceholder = options.langPlaceholder ?? '%paraglide.lang%'
  const dirPlaceholder =
    options.textDirectionPlaceholder ?? '%paraglide.textDirection%'
  return async ({ resolve, event }) => {
    ...
    const cookieLang = i18n2.runtime.isAvailableLanguageTag(langCookie)
      ? langCookie
      : void 0
    const negotiatedLanguagePreferences = negotiateLanguagePreferences(
      event.request.headers.get('accept-language'),
      i18n2.runtime.availableLanguageTags,
    )
      ...
      return await resolve(event, {
        transformPageChunk({ html, done }) {
          ...
          return html
            ...
            .replace(dirPlaceholder, textDirection)
        },
      })
    })
  }
...
function PrefixStrategy(
  ...
  function getLanguageFromLocalisedPath(localisedPath) {
    ...
    if (
      availableLanguageTags.includes(maybeLang) &&
  ...
  function getLocalisedPath(canonicalPath, languageTag) {
    ...
    let translatedPath = turnIntoTranslatedPath(
      canonicalPath,
      languageTag,
      translations,
      matchers,
  ...
  function getCanonicalPath(localisedPath, languageTag) {
    const trailingSlahsBefore =
      localisedPath.endsWith('/') && localisedPath !== '/'
    if (
      prefixDefaultLanguage === 'always' ||
      languageTag !== defaultLanguageTag
    ) {
    ...
    let canonicalPath = turnIntoCanonicalPath(
      localisedPath,
      languageTag,
      translations,
      matchers,
...
function createI18n(runtime2, options) {
  ...
  const strategy = PrefixStrategy(
    runtime2.availableLanguageTags,
    defaultLanguageTag,
    config.translations,
    config.matchers,
  ...
  return {
    ...
    getLanguageFromUrl(url) {
      ...
      return (
        strategy.getLanguageFromLocalisedPath(localizedPath) ||
    ...
    route(translatedPath) {
      const normalizedBase = normaliseBase(base)
      const [localisedPath, dataSuffix] = parseRoute(
        translatedPath,
        normalizedBase,
      )
      const lang = strategy.getLanguageFromLocalisedPath(localisedPath)
...
function normaliseBase(base2) {
  if (base2 === '') return ''
  if (base2.startsWith('/')) return base2
