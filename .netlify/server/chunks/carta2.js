...
const MathIcon = create_ssr_component(
    ...
    shortcuts: [
      {
        id: 'inlineKatex',
        combination:
          options?.block?.shortcut ?? /* @__PURE__ */ new Set(['control', 'm']),
        action: (input) => input.toggleSelectionSurrounding('$'),
      },
      {
        id: 'blockKatex',
        combination:
          options?.block?.shortcut ??
          /* @__PURE__ */ new Set(['control', 'shift', 'm']),
        action: (input) => input.toggleSelectionSurrounding(['$$\n', '\n$$']),
      },
    ],
    grammarRules: [
...
class TextAreaHistory {
   ...
   */
  undo() {
    if (-this.currentIndex > this.states.length) return
   ...
   */
  getSize = () =>
    this.states.reduce((acc, curr) => acc + curr.value.length * 2, 0)
  /**
  ...
  saveState(value, cursor) {
    ...
    if (
      latest &&
      Date.now() - latest.timestamp.getTime() <=
        (this.options.minInterval ?? 300)
    ) {
      this.states.pop()
    }
...
class InputEnhancer {
  ...
  handleMouseDown(e) {
    ...
    if (e.detail == 2 && currentChar != '\n' && currentChar != ' ') {
      ...
      while (
        startPosition >= 0 &&
        this.isWordCharacter(this.textarea.value[startPosition]) ==
          isWordChar &&
  ...
  handleKeyDown(e) {
    ...
    if (key === 'enter') {
      ...
      const tabOutsDelimiters = this.settings.tabOuts
        .map((tabOut) => tabOut.delimiter)
        .flat()
  ...
  handleKeyUp(e) {
    ...
    if (
      this.onKeyDownValue !== void 0 &&
      this.textarea.value != this.onKeyDownValue
    ) {
      this.history.saveState(this.textarea.value, this.textarea.selectionStart)
    }
  }
  handleNewLine(e) {
    ...
    for (
      ...
      lineStartingIndex > 0 &&
      this.textarea.value.at(lineStartingIndex - 1) !== '\n';
      lineStartingIndex--
    );
  ...
  insertAt(position, string) {
    ...
    this.textarea.value =
      value.slice(0, position) + string + value.slice(position)
  }
  /**
  ...
  removeAt(position, count = 1) {
    const value = this.textarea.value
    this.textarea.value =
      value.slice(0, position) + value.slice(position + count)
  }
  /**
  ...
  toggleLinePrefix(prefix, whitespace = 'attach') {
    ...
    while (furtherLinesExist) {
      const currentPrefix = this.textarea.value.slice(
        index,
        index + prefix.length,
      )
      if (currentPrefix === prefix) {
        if (
          whitespace === 'attach' &&
          this.textarea.value.at(index + prefix.length) === ' '
        ) {
      ...
      while (
        index < this.textarea.value.length &&
        this.textarea.value.at(index) !== '\n'
      )
        index++
  ...
  $bindToCaret(elem, data) {
    ...
    const themeClass = Array.from(data.editorElement?.classList ?? []).find(
      (c) => c.startsWith('carta-theme__'),
    )
...
const HeadingIcon = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
...
const ItalicIcon = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
...
const BoldIcon = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
...
const LinkIcon = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
...
const ListBulletedIcon = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
...
const ListNumberedIcon = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
...
const ListTaskIcon = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
...
const CodeIcon = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
...
const StrikethroughIcon = create_ssr_component(
  ($$result, $$props, $$bindings, slots) => {
...
class Carta {
  ...
  constructor(options) {
    ...
    this.textareaListeners = listeners.filter(
      (it) => !cartaEvents.includes(it[0]),
    )
    ...
    this.icons.unshift(
      ...defaultIcons.filter((icon) =>
        options?.disableIcons === true
          ? false
          : !options?.disableIcons?.includes(icon.id),
      ),
    )
    this.prefixes.push(
      ...defaultPrefixes.filter((prefix) =>
        options?.disablePrefixes === true
          ? false
          : !options?.disablePrefixes?.includes(prefix.id),
      ),
    )
    this.tabOuts.push(
      ...defaultTabOuts.filter((tabOut) =>
        options?.disableTabOuts === true
          ? false
          : !options?.disableTabOuts?.includes(tabOut.id),
      ),
    )
  ...
  setupSynchronousProcessor({ gfmOptions }) {
    ...
    const remarkPlugins = this.mSyncTransformers.filter(
      (it) => it.type === 'remark',
    )
    const rehypePlugins = this.mSyncTransformers.filter(
      (it) => it.type === 'rehype',
    )
  ...
  async setupAsynchronousProcessor({ gfmOptions }) {
    ...
    const remarkPlugins = [
      ...this.mSyncTransformers,
      ...this.mAsyncTransformers,
    ].filter((it) => it.type === 'remark')
    const rehypePlugins = [
      ...this.mSyncTransformers,
      ...this.mAsyncTransformers,
    ].filter((it) => it.type === 'rehype')
  ...
  async render(markdown) {
    ...
    this.dispatcher.dispatchEvent(
      new CustomEvent('carta-render', { detail: { carta: this } }),
    )
  ...
  renderSSR(markdown) {
    ...
    this.dispatcher.dispatchEvent(
      new CustomEvent('carta-render-ssr', { detail: { carta: this } }),
    )
  ...
  bindToCaret(element, portal = document.querySelector('body')) {
    let callback
    if (this.input)
      callback = this.input.$bindToCaret(element, {
        portal,
        editorElement: this.element,
      }).destroy
    ...
    return {
      destroy: () => {
        callback && callback()
        this.elementsToBind = this.elementsToBind.filter(
          (it) => it.elem != element,
        )
      },
    }
...
export { Carta as C, debounce as d, math as m }
