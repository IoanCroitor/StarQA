import { d as derived, w as writable, r as readonly } from './index5.js'
...
const passwords_dont_match$1 = /* @__NO_SIDE_EFFECTS__ */ () =>
  `Passwords don't match`
const p_must_be_at_least_8_characters$1 = /* @__NO_SIDE_EFFECTS__ */ () =>
  `Password must be at least 8 characters long`
const p_must_contain_at_least_one_lowercase_letter$1 =
  /* @__NO_SIDE_EFFECTS__ */ () =>
    `Password must contain at least one lowercase letter`
const p_must_contain_at_least_one_uppercase_letter$1 =
  /* @__NO_SIDE_EFFECTS__ */ () =>
    `Password must contain at least one uppercase letter`
const p_must_contain_at_least_one_number$1 = /* @__NO_SIDE_EFFECTS__ */ () =>
  `Password must contain at least one number`
const p_must_contain_at_least_one_special_character$1 =
  /* @__NO_SIDE_EFFECTS__ */ () =>
    `Password must contain at least one special character`
const must_be_a_string$1 = /* @__NO_SIDE_EFFECTS__ */ () => `Must be a string`
const must_be_a_valid_email$1 = /* @__NO_SIDE_EFFECTS__ */ () =>
  `Must be a valid email`
const p_must_be_a_string$1 = /* @__NO_SIDE_EFFECTS__ */ () =>
  `Password must be a string`
const passwords_dont_match = /* @__NO_SIDE_EFFECTS__ */ (
  params = {},
  options = {},
) => {
  return {
...
const p_must_be_at_least_8_characters = /* @__NO_SIDE_EFFECTS__ */ (
  params = {},
  options = {},
) => {
  return {
...
const p_must_contain_at_least_one_lowercase_letter =
...
const p_must_contain_at_least_one_uppercase_letter =
  /* @__NO_SIDE_EFFECTS__ */ (params = {}, options = {}) => {
    return {
...
const p_must_contain_at_least_one_number = /* @__NO_SIDE_EFFECTS__ */ (
  params = {},
  options = {},
) => {
  return {
...
const p_must_contain_at_least_one_special_character =
  /* @__NO_SIDE_EFFECTS__ */ (params = {}, options = {}) => {
    return {
...
const must_be_a_string = /* @__NO_SIDE_EFFECTS__ */ (
  params = {},
  options = {},
) => {
  return {
...
const must_be_a_valid_email = /* @__NO_SIDE_EFFECTS__ */ (
  params = {},
  options = {},
) => {
  return {
...
const p_must_be_a_string = /* @__NO_SIDE_EFFECTS__ */ (
  params = {},
  options = {},
) => {
  return {
...
function pathExists(obj, path, options = {}) {
  if (!options.modifier) {
    options.modifier = (pathData) =>
      isInvalidPath(path, pathData) ? void 0 : pathData.value
  }
...
function comparePaths(newObj, oldObj) {
  ...
  function checkPath(data, compareTo) {
    ...
    if (isBuiltin(data.value)) {
      if (
        !isBuiltin(otherData?.value) ||
        builtInDiff(data.value, otherData.value)
      ) {
        return addDiff()
      }
    }
...
function schemaInfo(schema, isOptional, path) {
  ...
  const properties =
    schema.properties && types.includes('object')
      ...
      : void 0
  const union = unionInfo(schema)?.filter(
    (u) => u.type !== 'null' && u.const !== null,
  )
  return {
...
function _defaultValues(schema, isOptional, path) {
  ...
  if (!objectDefaults && info.union) {
    const singleDefault = info.union.filter(
      (s) => typeof s !== 'boolean' && s.default !== void 0,
    )
  ...
  if (isMultiTypeUnion()) {
    throw new SchemaError(
      'Default values cannot have more than one type.',
      path,
    )
...
function defaultValue(type, enumType) {
  switch (type) {
    ...
    default:
      throw new SchemaError(
        'Schema type or format not supported, requires explicit default value: ' +
          type,
      )
  }
}
  ...
  if (info.properties) {
    for (const [key, value] of Object.entries(info.properties)) {
      ...
      output[key] = _defaultTypes(
        info.properties[key],
        !info.required?.includes(key),
        [...path, key],
      )
    }
  }
  if (info.additionalProperties && info.types.includes('object')) {
    const additionalInfo = schemaInfo(
...
function mapErrors(errors, shape) {
  ...
  for (const error of errors) {
    ...
    const isLastIndexNumeric = /^\d$/.test(
      String(error.path[error.path.length - 1]),
    )
    const objectError =
      !isLastIndexNumeric &&
      pathExists(
...
function replaceInvalidDefaults(Data, Defaults, _schema, Errors, preprocessed) {
  const defaultType =
    ...
    typeof _schema.additionalProperties == 'object'
  ...
  function Types_correctValue(dataValue, defValue, type) {
    ...
    if (
      !types.length ||
      types.every((t) => t == 'undefined' || t == 'null' || t == 'any')
    ) {
      return dataValue
    ...
    for (const schemaType of types) {
      ...
      const sameExistance =
        sameType && (dataValue === null) === (defaultTypeValue === null)
  ...
  function Defaults_traverseAndReplace(defaultPath) {
    ...
    if (
      ...
      preprocessed?.includes(currentPath[0])
    )
      return
    const dataPath = pathExists(Data, currentPath)
    if (
      (!dataPath && defaultPath.value !== void 0) ||
      (dataPath && dataPath.value === void 0)
    ) {
      ...
      Data_setValue(
        currentPath,
        Types_correctValue(dataValue, defValue, fieldType),
      )
    }
  }
  {
...
function setCustomValidityForm(formElement, errors) {
  for (const el of formElement.querySelectorAll(
    ...
    if (
      ('dataset' in el && noCustomValidityDataAttribute in el.dataset) ||
      !el.name
    ) {
      continue
    }
...
function superFieldProxy(superForm2, path, baseOptions) {
  ...
  return {
    ...
    update(upd, options) {
      form.update(
        (data) => updateProxyField(data, path2, upd),
        options ?? baseOptions,
      )
    },
    set(value, options) {
      form.update(
        (data) => updateProxyField(data, path2, () => value),
        options ?? baseOptions,
      )
    },
  }
...
function _schemaShape(schema, path) {
  ...
  return info.types.includes('array') || info.types.includes('object')
    ? {}
    : void 0
}
...
function superForm(form, formOptions) {
  ...
  async function Form_clientValidation(event, force = false, adapter) {
    ...
    if (!force) {
      if (
        options.validationMethod == 'onsubmit' ||
  ...
  function Form__clearCustomValidity(formElement) {
    ...
    if (options.customValidity && formElement) {
      for (const el of formElement.querySelectorAll(`[name]`)) {
        ...
        const message =
          'validationMessage' in el ? String(el.validationMessage) : ''
  ...
  async function Form__displayNewErrors(errors, event, force) {
    ...
    traversePaths(errors, (error) => {
      ...
      function addError() {
        ...
        if (
          options.customValidity &&
          isEventError &&
          validity.has(joinedPath)
        ) {
      ...
      if (multiple) {
        const errorPath = pathExists(
          get_store_value(Errors),
          error.path.slice(0, -1),
        )
  ...
  async function Form_updateFromValidation(form2, successResult) {
    if (
      form2.valid &&
      successResult &&
      Form_shouldReset(form2.valid, successResult)
    ) {
  ...
  function Tainted_update(newData, taintOptions) {
    ...
    const newTainted = comparePaths(newData, Tainted.clean).map((path) =>
      path.join(),
    )
    if (paths.length) {
      if (taintOptions == 'untaint-all' || taintOptions == 'untaint-form') {
        ...
        Tainted.state.update((currentlyTainted) => {
          ...
          setPaths(currentlyTainted, paths, (path, data) => {
            ...
            return currentValue &&
              cleanPath &&
              currentValue.value === cleanPath.value
              ? void 0
              : taintOptions === true
                ? true
  ...
  const Unsubscriptions = [
    ...
    Constraints.subscribe(
      (constraints2) => (__data.constraints = constraints2),
    ),
  ...
  function rebind(opts) {
    ...
    if (opts.untaint || opts.resetted) {
      Tainted_set(
        typeof opts.untaint === 'boolean' ? void 0 : opts.untaint,
        form2.data,
      )
    }
    ...
    return enhance(FormElement, async (submitParams) => {
      ...
      const submit = {
        ...
        jsonData(data) {
          if (options.dataType !== 'json') {
            throw new SuperFormError(
              "options.dataType must be set to 'json' to use jsonData.",
            )
          }
          jsonData = data
        },
      ...
      if (!cancelled) {
        ...
        if (!cancelled) {
          ...
          const submitData =
            'formData' in submit ? submit.formData : submit.data
          lastInputChange = void 0
          if (Form_isSPA()) {
          ...
          if (
            ...
            for (const newForm of forms) {
              ...
              if (!cancelled2) {
                ...
                if (
                  Form_shouldReset(data2.form.valid, result.type == 'success')
                ) {
                  data2.formElement
  ...
  return {
    ...
    async validate(path, opts = {}) {
      if (!options.validators) {
        throw new SuperFormError(
          'options.validators must be set to use the validate method.',
        )
      }
      ...
      if (opts.update && enhancedForm) {
        setTimeout(() => {
          ...
          scrollToFirstError(enhancedForm, {
            ...options,
            scrollToError:
              opts.focusOnError === false ? 'off' : options.scrollToError,
          })
        }, 1)
      }
...
function _constraints(info, path) {
  ...
  if (info.union && info.union.length) {
    ...
    if (
      ...
      (info.isNullable ||
        info.isOptional ||
        infos.some((i) => i?.isNullable || i?.isOptional))
    ) {
  ...
  if (info.array) {
    output = merge(
      output,
      ...info.array.map((i) =>
        _constraints(schemaInfo(i, info.isOptional, path), path),
      ),
    )
  }
  if (info.properties) {
    ...
    for (const [key, prop] of Object.entries(info.properties)) {
      const propInfo = schemaInfo(
        prop,
        !info.required?.includes(key) || prop.default !== void 0,
        [key],
      )
      const propConstraint = _constraints(propInfo, [...path, key])
      if (
        typeof propConstraint === 'object' &&
        Object.values(propConstraint).length > 0
      ) {
...
function constraint(info) {
  ...
  if (type == 'integer' && format == 'unix-time') {
    ...
    const patterns = [
      str.pattern,
      ...(str.allOf
        ? str.allOf.map((s) => (typeof s == 'boolean' ? void 0 : s.pattern))
        : []),
    ].filter((s) => s !== void 0)
    ...
    if (num.minimum !== void 0) output.min = num.minimum
      ...
      output.min =
        num.exclusiveMinimum + (type == 'integer' ? 1 : Number.MIN_VALUE)
    if (num.maximum !== void 0) output.max = num.maximum
    else if (num.exclusiveMaximum !== void 0)
      output.max =
        num.exclusiveMaximum - (type == 'integer' ? 1 : Number.MIN_VALUE)
...
function _schemaHash(info, depth, path) {
  ...
  function mapSchemas(schemas) {
    return schemas
      .map((s) =>
        _schemaHash(
          schemaInfo(s, info?.isOptional ?? false, path),
          depth + 1,
          path,
        ),
      )
    ...
    if (formData.has('__superform_json')) {
      try {
        ...
        if (typeof output === 'object') {
          ...
          for (const path of filePaths.filter((path2) =>
            path2.startsWith('__superform_file_'),
          )) {
            const realPath = splitPath(path.substring(17))
            setPaths(output, [realPath], formData.get(path))
          }
          for (const path of filePaths.filter((path2) =>
            path2.startsWith('__superform_files_'),
          )) {
...
function _parseFormData(formData, schema, options) {
  ...
  if (options?.strict) {
    schemaKeys = new Set(
      [...formData.keys()].filter((key) => !key.startsWith('__superform_')),
    )
  } else {
    let unionKeys = []
    if (schema.anyOf) {
      ...
      if (info.union?.some((s) => s.type !== 'object')) {
        throw new SchemaError(
          'All form types must be an object if schema is a union.',
        )
      }
      unionKeys =
        info.union?.flatMap((s) => Object.keys(s.properties ?? {})) ?? []
    }
    schemaKeys = new Set(
      [
        ...unionKeys,
  ...
  function parseSingleEntry(key, entry, info) {
    ...
    if (entry && typeof entry !== 'string') {
      const allowFiles = legacyMode
        ? options?.allowFiles === true
        : options?.allowFiles !== false
      return !allowFiles
        ? void 0
        : entry.size
          ? entry
  ...
  for (const key of schemaKeys) {
    ...
    if (info.types.includes('array') || info.types.includes('set')) {
      const items =
        property.items ?? (info.union?.length == 1 ? info.union[0] : void 0)
      if (
        !items ||
      ...
      const isFileArray =
        entries.length && entries.some((e) => e && typeof e !== 'string')
...
function _zod(schema, options) {
  return /* @__PURE__ */ createAdapter({
    ...
    jsonSchema:
      options?.jsonSchema ??
      /* @__PURE__ */ zodToJSONSchema(schema, options?.config),
...
const loginSchema = z.object({
  email: z
    .string({ message: /* @__PURE__ */ must_be_a_string() })
    .email({ message: /* @__PURE__ */ must_be_a_valid_email() }),
  password: z
    .string({ message: /* @__PURE__ */ p_must_be_a_string() })
    .min(8, { message: /* @__PURE__ */ p_must_be_at_least_8_characters() }),
})
const registerSchema = z
  .object({
    ...
    password: z
      ...
      .regex(/\d/, {
        message: /* @__PURE__ */ p_must_contain_at_least_one_number(),
      })
      .regex(/[\W_]/, {
        message:
          /* @__PURE__ */ p_must_contain_at_least_one_special_character(),
      }),
