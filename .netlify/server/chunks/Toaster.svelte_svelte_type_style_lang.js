...
function createToastState() {
  ...
  function dismiss(id) {
    if (id === void 0) {
      toasts.update((prev) =>
        prev.map((toast2) => ({ ...toast2, dismiss: true })),
      )
      return
    }
    toasts.update((prev) =>
      prev.map((toast2) =>
        toast2.id === id ? { ...toast2, dismiss: true } : toast2,
      ),
    )
    return id
  }
  ...
  function setHeight(data) {
    const exists = get_store_value(heights).find(
      (el) => el.toastId === data.toastId,
    )
    if (exists === void 0) {
...
export { toastState as a, cn as c, toast as t, useEffect as u }
